...
|----------|----------|--------------|----------|
| Fortificant 导致 pseudo multi-ingredient | Yogurt、orange juice、shredded wheat 等去 fortificant 后只剩一个 base food | 在 export 阶段增加 `single_base_with_fortification` 分类；base food 直接设为 1.0，fortificant 进入单独 metadata，不进入普通 ingredient fraction 求解 | 这 6 类不再记为普通 export failure；报告中单独统计 |
| Fortified cereal | 强化营养素解释了主要标签差异，删除 fortificant 后信息不足 | 建立 fortificant→nutrient contribution 表；把 calcium、vitamin D、iron、B vitamins 等作为可解释 additive profile，而不是简单删除 | cereal export 成功率提升；被跳过 nutrient 数量下降 |
...
2. **把 post-hoc 修复变成自动 policy**：特别是 `max_iterations=2000` retry 和 fortificant filtering。
3. **再做 fortificant metadata**：这是 cereal 和 fortified food 的主要瓶颈。
